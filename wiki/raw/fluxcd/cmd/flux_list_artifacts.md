---
source_url: https://fluxcd.io/flux/cmd/flux_list_artifacts/
fetched: 2026-05-01
---

  1. [Docs](<https://fluxcd.io/flux/>)
  2. [Flux CLI](<https://fluxcd.io/flux/cmd/>)
  3. [flux list artifacts](<https://fluxcd.io/flux/cmd/flux_list_artifacts/>)
...
The list command fetches the tags and their metadata from a remote OCI repository. The command can read the credentials from '~/.docker/config.json' but they can also be passed with -creds. It can also login to a supported provider with the -provider flag.
```
flux list artifacts [flags]
```

### Examples
```
  # List the artifacts stored in an OCI repository
...
```
      ...
      --filter-regex string              filter tags returned from the oci repository using regex
      --filter-semver string             filter tags returned from the oci repository using semver
  -h, --help                             help for artifacts
      --insecure-registry                allows the remote artifacts list to be fetched without TLS
