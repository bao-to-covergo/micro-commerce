---
source_url: https://fluxcd.io/flux/installation/bootstrap/gitlab/
fetched: 2026-05-01
---
  ...
  4. [GitLab](<https://fluxcd.io/flux/installation/bootstrap/gitlab/>)



# Flux bootstrap for GitLab

How to bootstrap Flux with GitLab and GitLab Enterprise

The [flux bootstrap gitlab](</flux/cmd/flux_bootstrap_gitlab/>) command deploys the Flux controllers on a Kubernetes cluster and configures the controllers to sync the cluster state from a GitLab project. Besides installing the controllers, the bootstrap command pushes the Flux manifests to the GitLab project and configures Flux to update itself from Git.
...
When using `--deploy-token-auth`, the CLI generates a [GitLab project deploy token](<https://docs.gitlab.com/ee/user/project/deploy_tokens/>) and stores it in the cluster as a Kubernetes Secret named `flux-system` inside the `flux-system` namespace.
